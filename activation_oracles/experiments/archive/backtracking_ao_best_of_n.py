"""
Best-of-N AO skyline for backtracking eval with full context.

Runs the AO with 10 rollouts (temp=1) on full context, judges all,
picks the best per entry. Adds a skyline row to the context length sweep.

Usage:
    source .env && .venv/bin/python experiments/backtracking_ao_best_of_n.py
"""
...
NUM_ROLLOUTS = 10
FULL = 99999
OUTPUT_DIR = "experiments/backtracking_ao_best_of_10_full_context"
...
def main():
    ...
    print(f"\nRunning backtracking AO best-of-{NUM_ROLLOUTS} with full context")
    ...
    result = run_backtracking_open_ended_eval(
        ...
        multi_rollout_mode="best_of_n",
        segment_tokens=FULL,
    )
    elapsed = time.perf_counter() - start

    print(f"\n{'='*60}")
    print(f"RESULTS (best-of-{NUM_ROLLOUTS}, full context)")
    print(f"{'='*60}")
