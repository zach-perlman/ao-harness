"""One-off: 4-quadrant comparison of AO lineage — Original AO, ML AO, ML AO + On-Policy, AO v2."""
...
MODELS = {
    ...
    "codex_old_past_lens_300k_full_mix": "ML AO",
    "on_policy_Qwen3-8B": "ML AO + On-Policy",
    "full_mix_500k_past_lens_plus_synthetic_qa_v3": "AO v2",
}

BAR_COLORS = ["#4363d8", "#3cb44b", "#f58231", "#e6194b"]
...
def main():
    ...
    fig.legend(handles, leg_labels, loc="lower center", ncol=len(lora_order),
               fontsize=FONT_LEGEND + 2, bbox_to_anchor=(0.5, -0.01))

    plt.suptitle("AO Lineage — Original AO → ML AO → ML AO + On-Policy → AO v2",
                 fontsize=FONT_SUPTITLE, fontweight="bold", y=0.98)
    plt.tight_layout(rect=[0, 0.04, 1, 0.96])

    os.makedirs("experiments/eval_plots", exist_ok=True)
    out = "experiments/eval_plots/ao_lineage_comparison.png"
